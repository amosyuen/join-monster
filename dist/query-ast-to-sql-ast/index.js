...
function handleColumnsRequiredForSorting(sqlASTNode, fieldTypes, namespace) {
  ...
  if (sqlASTNode.sortKey || ((_ref2 = sqlASTNode) != null ? (_ref2 = _ref2.junction) != null ? _ref2.sortKey : _ref2 : _ref2)) {
    ...
    (0, _assert2.default)(sortKey.order || !sqlASTNode.paginate, '"sortKey" must have "order"');
    ...
    for (let column of (0, _util.wrap)((0, _util.ensure)(sortKey, 'key'))) {
      ...
      if (field && field.sqlExpr) {
        newChild = expressionToASTChild(column, field.sqlExpr, namespace, sqlASTNode.args, true);
    ...
    for (let column in orderBy) {
      const field = findFieldInTableOrJunction(sqlASTNode, fieldTypes, column, sqlASTNode.paginate);
      if (field && field.sqlExpr) {
        const newChild = expressionToASTChild(column, field.sqlExpr, namespace, sqlASTNode.args, true);
        if (sqlASTNode.junction) {
          if (!field) {
            newChild.fromOtherTable = sqlASTNode.junction.as;
          }
...
function findFieldInTableOrJunction(sqlASTNode, fields, column, paginate) {
  ...
  if (!field && sqlASTNode.junction) {
    ...
    (0, _assert2.default)(junctionField, 'We should always find the junction table');
    const joinedTableFields = paginate ? junctionField.type._fields.edges.type.ofType._fields.node.type._fields : junctionField.type.ofType._fields;
    (0, _assert2.default)(joinedTableFields, 'We should always find the joined table fields');
